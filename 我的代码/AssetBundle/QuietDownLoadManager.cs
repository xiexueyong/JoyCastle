...
namespace Framework.Asset
{
    public class QuietDownLoadManager : D_MonoSingleton<QuietDownLoadManager>
    {
        ...
        {
            return;
            if (AssetBundleConfig.Instance.UseAssetBundle && !_finishLoad)
            {
        ...
        private void QuietLoad()
        {
            ...
            //_mustLoadGroupNames = AssetBundleManager.Instance.GetNotBaseGroupNames();
            ...
            {
                if (!AssetBundleManager.Instance.IsBundleGroupLoaded(_mustLoadGroupNames[i]))
                {
                    DebugUtil.Log("QuietDownLoadManager : start load {0}", _mustLoadGroupNames[i]);
                    AssetBundleManager.Instance.LoadBundleGroup(_mustLoadGroupNames[i],true);
                    curAssetBundleGroupLoadHelper = AssetBundleManager.Instance.GetAssetBundleGroupLoadHelperItem(_mustLoadGroupNames[i]);
                    return;
                }
            }
            //预下载的
            ...
            {
                if (!AssetBundleManager.Instance.IsBundleGroupLoaded(_preLoadGroupNames[i]))
                {
                    DebugUtil.Log("QuietDownLoadManager : start load {0}", _preLoadGroupNames[i]);
                    AssetBundleManager.Instance.LoadBundleGroup(_preLoadGroupNames[i], true);
                    curAssetBundleGroupLoadHelper = AssetBundleManager.Instance.GetAssetBundleGroupLoadHelperItem(_preLoadGroupNames[i]);
                    return;
                }
            }
        ...
        {
            _preLoadGroupNames = AssetBundleManager.Instance.GetMustGroups();
        ...
        public void Update()
        {
            ...
            //}
            f++;
            if (_initQuietLoad && f > 180 && AssetBundleConfig.Instance.UseAssetBundle)
            {
                f = 0;
                CheckNetStatus();
            }
        ...
        public class QuietDownLoadInfo
        {
            ...
            {
                get{
                    if (Instance.curAssetBundleGroupLoadHelper != null && Instance.curAssetBundleGroupLoadHelper.progressInfo != null)
                    {
                        return Instance.curAssetBundleGroupLoadHelper.progressInfo.progress;
                    }
                    else
                    {
                        return 0;
