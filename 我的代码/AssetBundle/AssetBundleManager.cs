...
namespace Framework.Asset
{
    public class AssetBundleManager : D_MonoSingleton<AssetBundleManager>
    {
...
#if !UNITY_EDITOR
            AssetBundleConfig.Instance.UseAssetBundle = true;// 非编辑器模式下，永远使用ab包
#endif
        ...
        public IEnumerator Upgrade(GameController.UpgradeInfo upgradeInfo)
        {
            ...
            vb.appVer = AssetBundleConfig.Instance.appVersion;
            vb.assetVer = this._versionInfo.Version; 
            HttpRequestTool.SendMessage(vb,
                (sucessData) =>
                {
                    ...
                    if (upgradeInfo != null)
                    {
                        ...
                        if (upgradeInfo.appUpdateType == 1)
                        {
                            upgradeInfo.noticeTitle = jsonNode["appNotice"]["noticeTitle"];
                            upgradeInfo.noticeContent = jsonNode["appNotice"]["noticeContent"];
                            TableBI.Version_Updating(vb.appVer, upgradeInfo.appVersion, vb.assetVer, upgradeInfo.assetVersion);
                        }
        ...
        public List<string> GetMustGroups(List<PreloadConditionItem> preloadConditionItems)
        {
            ...
            while (preloadConditionItems != null && i < preloadConditionItems.Count)
            {
                ...
                if (preloadConditionItems[i].taskCondition == 10000 || DataManager.Instance.taskInfo.TaskIsFinshed(preloadConditionItems[i].taskCondition))
                {
