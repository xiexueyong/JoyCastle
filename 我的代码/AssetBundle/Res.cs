...
namespace Framework.Asset
{
    public class Res : D_MonoSingleton<Res>
    {
        ...
        /// <returns></returns>
        public static T LoadResource<T>(string name) where T : UObject
        {
            if (GameController.ApplicationQuit)
            {
                return null;
            }
            return Instance.actualLoadResource<T>(name);
        ...
        public static T LoadResourceWithoutCache<T>(string name) where T : UObject
        {
            ...
            return Instance._cachedEditorObjectContainer.LoadWithOutCache<T>(name);
        ...
        private void RegisterAsset(UObject assetObject, CachedObject cachedObject)
        {
        ...
        //{
        //    string guessBundleName = AssetBundleManager.Instance.GueessBundleNameByAssetName(originPath);
      ...
      /// <param name="delay">以秒为单位</param>
        protected void RecycleDelay(UObject uObject,float delay)
        {
            StartCoroutine(RecycleEnumerator(uObject,delay));
        }
        private IEnumerator RecycleEnumerator(UObject uObject, float delay)
        {
            yield return new WaitForSeconds(delay);
            actualRecycle(uObject);
        }
        public static bool Recycle(UObject uObject)
        {
            ...
            return Instance.actualRecycle(uObject);
        }
        ...
        public class ResEditor : Editor
        {
            ...
            private void OnBundleGui()
            {
                ...
                //Dictionary<string, CachedAssetBundle> cabDic = Res.Instance._cachedAssetBundleContainer.CachedAssetBundleDictionary;
                List<KeyValuePair<string, CachedAssetBundle>> cabList = Res.Instance._cachedAssetBundleContainer.CachedAssetBundleDictionary.ToList();
            ...
            {
                List<KeyValuePair<string, CachedObject>> CachedObjectList = Res.Instance._cachedEditorObjectContainer.CachedObjectDictionary.ToList();
